"""omni_office __main__ 入口 smoke 测试。

经 runpy 以 ``python -m omni_office`` 语义执行 __main__.py（cli.main 用 Mock 替换，
...
class TestDunderMain:
    def test_main_forwards_to_cli(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        monkeypatch.setattr("omni_office.cli.main", mock_main)
        with pytest.raises(SystemExit) as exc_info:
            runpy.run_module("omni_office", run_name="__main__", alter_sys=True)
...
class TestCliHelp:
    def test_help_exits_zero(self) -> None:
        ...
        from omni_office.cli import main
