"""omni_brightness：屏幕亮度控制插件（macOS ``brightness`` CLI 桥接）。
...
__all__ = ["BrightnessPlugin", "register"]
...
class BrightnessPlugin(SystemPluginBase):
    """omni_brightness 插件类，继承 SystemPluginBase。"""

    name: str = "omni_brightness"
    version: str = "0.1.0"
    description: str = "屏幕亮度控制插件 - macOS brightness CLI 桥接，支持设置/查询亮度"
    emoji: str = "🔆"
    ...
    async def on_load(self, ctx: PluginContext) -> None:
        ...
        self._logger.info("omni_brightness 插件已加载，注册 %d 个工具", len(tools.TOOLS))
