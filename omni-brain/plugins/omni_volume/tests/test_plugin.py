"""omni_volume VolumePlugin 生命周期测试 + manifest 校验。

验证：
- ``VolumePlugin`` 继承 ``OmniPlugin``，元数据 name="omni_volume"
- ``VolumePlugin.on_load(ctx)`` 把 4 个 system_* 工具注册到 ``ctx.tool_registry``
...
from omni_volume import VolumePlugin, register


VOLUME_DIR = Path(__file__).resolve().parent.parent
...
class TestVolumePluginMetadata:
    def test_volume_plugin_is_omni_plugin_subclass(self) -> None:
        """VolumePlugin 是 OmniPlugin 子类。"""
        assert issubclass(VolumePlugin, OmniPlugin)

    def test_volume_plugin_metadata(self) -> None:
        """VolumePlugin 元数据 name="omni_volume"。"""
        plugin = VolumePlugin()
        assert plugin.name == "omni_volume"
        assert plugin.version
        assert plugin.description
        assert plugin.emoji

    def test_volume_plugin_direct_subclass_not_compat(self) -> None:
        """VolumePlugin 直接继承 OmniPlugin，不经过 RegisterCompatPlugin。"""
        from omni_sdk.compat import LegacyPluginAdapter

        assert not issubclass(VolumePlugin, LegacyPluginAdapter)


class TestVolumePluginOnLoad:
    def test_on_load_calls_register(self) -> None:
        ...
        assert "system_set_volume" in tools
        assert "system_get_volume" in tools
        assert "system_mute" in tools
        assert "system_unmute" in tools
        assert len(tools) == 4
    ...
    def test_on_load_tool_has_schema(self) -> None:
        ...
        ctx = _make_plugin_context()
        asyncio.run(plugin.on_load(ctx))
        tool = ctx.tool_registry.get_tool("system_set_volume")
    ...
    def test_on_load_tool_handler_returns_json(self) -> None:
        ...
        ctx = _make_plugin_context()
        asyncio.run(plugin.on_load(ctx))
        tool = ctx.tool_registry.get_tool("system_get_volume")
        assert tool is not None
...
class TestVolumePluginBackwardCompat:
    ...
    def test_register_legacy_ctx_handler_returns_json(self) -> None:
        ...
        ctx = _LegacyCtx()
        register(ctx)
        tool = next(t for t in ctx.tools if t["name"] == "system_get_volume")
        result = tool["handler_func"]({"fake": True})
        data = json.loads(result)
        assert data["ok"] is True
...
class TestVolumeManifest:
    def test_manifest_json_exists(self) -> None:
        """omni_volume/manifest.json 文件存在。"""
        assert (VOLUME_DIR / "manifest.json").is_file()

    def test_manifest_json_parses(self) -> None:
        """manifest.json 经 parse_manifest 解析合法。"""
        data = json.loads((VOLUME_DIR / "manifest.json").read_text(encoding="utf-8"))
        manifest = parse_manifest(data)
        assert manifest.name == "omni_volume"
        assert manifest.version
        assert manifest.description
        errors = validate_manifest(manifest)
        # validate_manifest 返回软错误列表；权限 tools.register 已知，不应有未知前缀错误
    ...
    def test_manifest_declares_volume_changed_event(self) -> None:
        """manifest.events.publishes 声明 system.volume_changed。"""
        data = json.loads((VOLUME_DIR / "manifest.json").read_text(encoding="utf-8"))
        manifest = parse_manifest(data)
        assert "system.volume_changed" in manifest.events.publishes
