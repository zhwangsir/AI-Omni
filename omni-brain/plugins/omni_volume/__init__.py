"""omni_volume：系统音量控制插件（macOS osascript 桥接）。

继承 ``SystemPluginBase``（M15+ SDK），统一处理事件桥接与工具注册。

工具清单：
- ``system_set_volume``  ：设置系统音量 0-100
...
class VolumePlugin(SystemPluginBase):
    """omni_volume 插件类，继承 SystemPluginBase。"""

    name: str = "omni_volume"
    version: str = "0.1.0"
    description: str = "系统音量控制插件 - macOS osascript 桥接，支持设置/查询/静音/取消静音"
    emoji: str = "🔊"
    ...
    async def on_load(self, ctx: PluginContext) -> None:
        ...
        self._logger.info("omni_volume 插件已加载，注册 %d 个工具", len(tools.TOOLS))
